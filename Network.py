...
def policy_loss(y_true ,y_pred):
    ...
    log_p_g=tf.gather_nd(log_p,actions)
    ll=-adv*log_p_g
...
class ActorCritic(tf.keras.Model):
    def __init__(self,in_shape,n_actions,**kwargs):
        ...
        self.lay1=Conv2D(filters=8,kernel_size=4,strides=1,padding="valid",activation="relu")
        self.lay2=Conv2D(filters=12,kernel_size=3,strides=2,padding="same",activation="relu")
        self.lay3=Conv2D(filters=12,kernel_size=2,strides=2,padding="same",activation="relu")
        self.lay4=Conv2D(filters=24,kernel_size=3,strides=2,padding="same",activation="relu")
        self.lay5=Conv2D(filters=42,kernel_size=2,strides=2,padding="same",activation="relu")
        self.lay6=Conv2D(filters=1,kernel_size=1,strides=1,padding="same",activation="relu")
        self.flat=Flatten()
        self.head=Dense(100,activation="relu")
        self.head1=Dense(20,activation="relu")
        self.head2=Dense(20,activation="relu")
